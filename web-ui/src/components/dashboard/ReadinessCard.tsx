...
import { MetricRing, ContributorRow } from '../viz';
...
const VERDICT_MAP: Record<string, { label: string; color: string }> = {
  green:  { label: 'READY',    color: STATUS.green },
  yellow: { label: 'MODERATE', color: STATUS.yellow },
  red:    { label: 'REST',     color: STATUS.red },
};
...
/**
 * Whoop-style readiness card with one large status-colored ring and the verdict
 * word inside it (color-blind safe). Tapping opens an Oura-style contributor
 * breakdown (HRV, RHR, Sleep, TSB).
 */
  ...
  return (
    <>
      <section
        ...
        className="m-4 flex cursor-pointer select-none flex-col items-center rounded-2xl bg-bg-raised px-4 py-6 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
      >
        <div className="relative">
          <MetricRing
            ...
            size="lg"
          />
          {/* Verdict word inside the ring — the key a11y move (not color alone) */}
          <span
            className="pointer-events-none absolute inset-x-0 bottom-7 text-center text-[11px] font-bold uppercase tracking-widest"
        ...
        {reasoning && (
          <p
            className="mt-3 max-w-[34ch] text-center text-[13px] leading-snug text-slate-400"
        ...
        {confounds && confounds.length > 0 && (
          <span className="mt-3 inline-flex items-center gap-1 rounded-full bg-status-yellow/15 px-2.5 py-1 text-[12px] font-medium text-status-yellow">
          ...
          {contributors.length > 0 && (
            <div className="mb-5">
              ...
              <div className="flex flex-col gap-3">
                {contributors.map((c) => (
                  <ContributorRow key={c.label} label={c.label} score={c.score} color={config.color} />
                ))}
              </div>
            </div>
          )}
